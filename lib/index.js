...
function bump(version, noGit, filenames) {
  ...
  if (!version) {
    return readFileToString(filenames[0]).then(parseJson).then(function (manifest) {
      return "v" + (manifest.version || "0.0.0");
    });
  }
  if (noGit) {
...
function updateManifestVersion(filename, version) {
  return readFileToString(filename).then(function (inData) {
